...
description: >-
  Add citations to manuscript text by splitting passages into citable segments and searching
  academic databases for supporting references. Supports two modes:
  (1) CNS-strict (default): Nature Portfolio, AAAS Science family, and Cell Press journals only;
  (2) Universal: all peer-reviewed journals from CrossRef, with quality assessment.
  Use this skill whenever the user asks to input text and automatically get references, add
  citations to a paragraph/manuscript, find support for statements, create text-to-reference
  correspondence, "分段引用", "自动给出引用", "Nature系列引用", "CNS及子刊", "支撑文献",
  "补引用", "找引用", or export EndNote/RIS/ENW/Zotero RDF. Also trigger on general
  academic-writing citation needs such as adding references while writing a paper, finding
  sources/literature for a claim, building a reference list, and Chinese phrasings like
